...
const ContactFormFixed: React.FC<ContactFormProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPlz('');
      setOrt('');
      setNachricht(propertyTitle ? `Ich interessiere mich für die Immobilie: ${propertyTitle}` : '');
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="datenschutz" className="text-sm leading-relaxed cursor-pointer">
          Ich habe die <span className="text-primary hover:underline cursor-pointer">Datenschutzerklärung</span> gelesen und willige der Verarbeitung meiner Daten zum Zweck der Bearbeitung meiner Anfrage ein. *
        </Label>
      </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between">
              <span>Interesse an: {propertyTitle}</span>
              <Button
                variant="ghost"
                size="sm"
  ...
  // For non-dialog usage (like the contact page)
  return (
